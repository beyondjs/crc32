...
`crc32.test.mjs` imports `@beyond-js/crc32/main` under BEE Node from an Engine development server that compiles this package; `@beyond-js/kernel` must resolve from the working directory.

```sh
BEE_URL=<server> BEE_ADAPTER=engine node --import "$BEE_NODE_DIR/register.mjs" --test tests/*.test.mjs
```

Inside the Beyond Suite, `node utils/validation/run.mjs crc32` prepares the server and runs it.
...
There are no fixture files. The inputs are short strings and byte buffers written inline, with their expected checksums, including the standard check vector of `"123456789"`.
